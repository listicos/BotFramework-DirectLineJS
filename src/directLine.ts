...
export class DirectLine implements IBotConnection {
  ...
  private checkConnection(once = false) {
    let obs = this.connectionStatus$
      ...
      .filter(
        (connectionStatus) =>
          connectionStatus != ConnectionStatus.Uninitialized && connectionStatus != ConnectionStatus.Connecting
      )
      .flatMap((connectionStatus) => {
    ...
    return (
      Observable.ajax({
        ...
        .retryWhen((error$) =>
          ...
          error$
            .mergeMap(
              (error) => (error.status >= 400 && error.status < 600 ? Observable.throw(error) : Observable.of(error))
            )
            .delay(timeout)
            .take(retries)
        )
